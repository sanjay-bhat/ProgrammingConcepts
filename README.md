...
|------------|---------------|----------------|---------------------------|-------|
| 1 | [📓 Two Sum](leetcode/0001.ipynb) | <span title="Store complements (or seen elements) in a hash map; find a match in the same forward pass.">One-Pass Hash Map</span> | Time: O(n), Space: O(n) | Easy |
| 2 | [📓 Add Two Numbers](leetcode/0002.ipynb) | <span title="Walk two linked-list digits simultaneously, carrying the overflow to the next position.">Iterative Addition</span> | Time: O(max(m, n)), Space: O(max(m, n)) | Medium |
| 3 | [📓 Longest Substring Without Repeating Characters](leetcode/0003.ipynb) | <span title="Expand/shrink a window tracking character frequencies in a hash map to satisfy a constraint.">Sliding Window with HashMap</span> | Time: O(n), Space: O(min(n, m)) | Medium |
| 4 | [📓 Median of Two Sorted Arrays](leetcode/0004.ipynb) | <span title="Binary search the partition point in two sorted arrays so left halves together equal right halves.">Binary Search on Partitions</span> | Time: O(log(min(m, n))), Space: O(1) | Hard |
| 5 | [📓 Longest Palindromic Substring](leetcode/0005.ipynb) | <span title="Expand palindrome outward from each center (single char or between chars); track the longest.">Expand Around Center</span> | Time: O(n^2), Space: O(n^2) | Medium |
| 6 | [📓 Zigzag Conversion](leetcode/0006.ipynb) | <span title="Process the matrix one row at a time, applying local rules to update state.">Row-by-Row Simulation</span> | Time: O(n), Space: O(n) | Medium |
| 7 | [📓 Reverse Integer](leetcode/0007.ipynb) | <span title="Extract digits with mod 10 and integer division; reassemble or compare without converting to string.">Math Digit Extraction</span> | Time: O(log x), Space: O(1) | Easy |
| 8 | [📓 String to Integer (atoi)](leetcode/0008.ipynb) | <span title="Single left-to-right scan; update a running answer (min, max, count, etc.) at each step.">Linear Scanning</span> | Time: O(n), Space: O(1) | Easy |
| 9 | [📓 Palindrome Number](leetcode/0009.ipynb) | <span title="Reverse the second half of the digits and compare to the first half to detect palindromes.">Reverse Half the Number</span> | Time: O(log x), Space: O(1) | Easy |
| 10 | [📓 Regular Expression Matching](leetcode/0010.ipynb) | <span title="Fill a DP table iteratively from base cases upward, avoiding recursion and call-stack overhead.">Bottom-Up DP</span> | Time: O(m * n), Space: O(m * n) | Hard |
...
| 12 | [📓 Integer to Roman](leetcode/0012.ipynb) | <span title="Map each Roman symbol to its value; subtract when a smaller symbol precedes a larger one.">Greedy Symbol Table</span> | Time: O(1), Space: O(1) | Medium |
| 13 | [📓 Roman to Integer](leetcode/0013.ipynb) | <span title="Scan left-to-right; look one position ahead to decide whether to add or subtract the current symbol.">Left-to-Right Lookahead</span> | Time: O(n), Space: O(1) | Easy |
| 14 | [📓 Longest Common Prefix](leetcode/0014.ipynb) | <span title="Compare characters column by column across all strings; stop at the first mismatch.">Vertical Scanning</span> | Time: O(n * m), Space: O(1) | Easy |
| 15 | [📓 3Sum](leetcode/0015.ipynb) | <span title="Sort first, then use two pointers from both ends to find pairs or triplets in a single sweep.">Sorting + Two Pointers</span> | Time: O(n^2), Space: O(1) | Medium |
| 16 | [📓 3Sum Closest](leetcode/0016.ipynb) | <span title="Sort first, then use two pointers from both ends to find pairs or triplets in a single sweep.">Sorting + Two Pointers</span> | Time: O(n^2), Space: O(1) | Medium |
| 17 | [📓 Letter Combinations of a Phone Number](leetcode/0017.ipynb) | <span title="Explore all candidates recursively; undo (backtrack) a choice when it leads to a dead end.">Backtracking</span> | Time: O(4^n · n), Space: O(n) | Medium |
...
| 44 | [📓 Wildcard Matching](leetcode/0044.ipynb) | <span title="Roll the DP match table to a single row; propagate '*' horizontally and '?'/letter diagonally.">1D DP</span> | Time: O(m * n), Space: O(m * n) | Hard |
...
| 53 | [📓 Maximum Subarray](leetcode/0053.ipynb) | <span title="Track running subarray sum; reset to current element when the accumulated sum drops below it.">Kadane's Algorithm</span> | Time: O(n), Space: O(1) | Easy |
...
| 62 | [📓 Unique Paths](leetcode/0062.ipynb) | <span title="Count paths as a binomial coefficient C(m+n-2, m-1); compute in O(min(m,n)) with O(1) space.">Math (Combinatorics)</span> | Time: O(m * n), Space: O(m * n) | Medium |
| 63 | [📓 Unique Paths II](leetcode/0063.ipynb) | <span title="Process the obstacle grid row by row with a 1D array; zero out obstacle cells immediately.">1D DP</span> | Time: O(m * n), Space: O(m * n) | Medium |
| 64 | [📓 Minimum Path Sum](leetcode/0064.ipynb) | <span title="Carry the minimum-cost row forward; each cell takes the cheaper of above or left predecessor.">1D DP</span> | Time: O(m * n), Space: O(m * n) | Medium |
...
| 70 | [📓 Climbing Stairs](leetcode/0070.ipynb) | <span title="Fibonacci-style DP: ways(n) = ways(n-1) + ways(n-2); track only two variables at a time.">Space-Optimized DP</span> | Time: O(n), Space: O(1) | Easy |
| 71 | [Simplify Path](https://leetcode.com/problems/simplify-path/) | Stack | Time: O(n), Space: O(n) | Medium |
| 72 | [📓 Edit Distance](leetcode/0072.ipynb) | <span title="Roll the edit-distance table to one row; track the diagonal predecessor in a temp variable.">1D DP</span> | Time: O(m * n), Space: O(m * n) | Hard |
...
| 87 | [📓 Scramble String](leetcode/0087.ipynb) | <span title="Recurse on (i1, i2, length) sub-problems; prune with a character-frequency check before branching.">Memoized Recursion</span> | Time: O(n^4), Space: O(n^4) | Hard |
...
| 91 | [📓 Decode Ways](leetcode/0091.ipynb) | <span title="Fibonacci-style DP over the digit string; two variables track single- and two-digit decode counts.">Space-Optimized DP</span> | Time: O(n), Space: O(n) | Medium |
...
| 96 | [📓 Unique Binary Search Trees](leetcode/0096.ipynb) | <span title="dp[i] = sum of dp[k-1]*dp[i-k] for k in 1..i; the answer is the n-th Catalan number.">Catalan Number DP</span> | Time: O(n^2), Space: O(n) | Medium |
| 97 | [📓 Interleaving String](leetcode/0097.ipynb) | <span title="Roll the interleaving DP table to one row; update from above (s1 char) or left (s2 char).">1D DP</span> | Time: O(m * n), Space: O(m * n) | Hard |
...
| 1199 | [📓 Minimum Time to Build Blocks](leetcode/1199.ipynb) | <span title="Repeatedly merge the two cheapest build-times via a min-heap; the last element is the minimum bottleneck time.">Huffman Greedy (Min-Heap)</span> | Time: O(n log n), Space: O(n) | Hard |
...
| 1215 | [📓 Stepping Numbers](leetcode/1215.ipynb) | <span title="Seed BFS with digits 1-9 and expand each number by appending last_digit ± 1; collect those within [low, high].">BFS Digit Expansion</span> | Time: O(K), Space: O(K) | Medium |
...
| 1220 | [📓 Count Vowels Permutation](leetcode/1220.ipynb) | <span title="Track five counts (one per vowel) and propagate via fixed adjacency rules each step; only O(1) state needed.">Transition-Matrix DP</span> | Time: O(n), Space: O(1) | Hard |
...
| 1235 | [📓 Maximum Profit in Job Scheduling](leetcode/1235.ipynb) | <span title="Sort jobs by end time; for each job binary-search the latest compatible predecessor and pick skip-or-take greedily.">DP + Binary Search</span> | Time: O(n log n), Space: O(n) | Hard |
...
| 1240 | [📓 Tiling a Rectangle with the Fewest Squares](leetcode/1240.ipynb) | <span title="Fill the lowest skyline column first with the largest valid square; prune branches exceeding the current best.">Backtracking + Skyline Pruning</span> | Time: O(min(m,n)^(mn)), Space: O(mn) | Hard |
...
| 1262 | [📓 Greatest Sum Divisible by Three](leetcode/1262.ipynb) | <span title="Maintain three running best-sums keyed by remainder mod 3; update each element in O(1) with a snapshot swap.">Remainder-State DP</span> | Time: O(n), Space: O(1) | Medium |
...
| 1340 | [📓 Jump Game V](leetcode/1340.ipynb) | <span title="Process bars shortest-first so all reachable (lower) bars are solved; scan ±d steps and take the max sub-result.">Sort + Memoized DP</span> | Time: O(n·d), Space: O(n) | Hard |
...
| 1388 | [📓 Pizza With 3n Slices](leetcode/1388.ipynb) | <span title="Pick exactly n non-adjacent elements from a circular 3n array; run two House Robber DPs to handle circularity.">Circular House Robber DP</span> | Time: O(n^2), Space: O(n^2) | Hard |
...
| 1400 | [📓 Construct K Palindrome Strings](leetcode/1400.ipynb) | <span title="Count characters with odd frequency; answer is true iff odd_count <= k <= n (each odd-freq char needs its own center).">Odd-Frequency Count</span> | Time: O(n), Space: O(1) | Medium |
...
| 1406 | [📓 Stone Game III](leetcode/1406.ipynb) | <span title="Compute score advantage from each position right-to-left; taking 1-3 stones, the current player maximises own lead.">Suffix DP</span> | Time: O(n), Space: O(1) | Hard |
...
| 1420 | [📓 Build Array Where You Can Find The Maximum Exactly K Comparisons](leetcode/1420.ipynb) | <span title="3D DP over (position, current_max, comparisons_used); transition by placing a value <= max or a new larger max.">3D DP</span> | Time: O(n·k·m^2), Space: O(n·k·m) | Hard |
...
| 1463 | [📓 Cherry Pickup II](leetcode/1463.ipynb) | <span title="Two robots descend row by row; dp[r][c1][c2] = max cherries with robot positions c1, c2; rolling array saves space.">3D DP</span> | Time: O(rows·cols^2), Space: O(cols^2) | Hard |
...
| 1473 | [📓 Paint House III](leetcode/1473.ipynb) | <span title="dp[i][color][neighborhoods] = min cost; transition checks whether the new color matches the previous to increment neighborhoods.">3D DP</span> | Time: O(n·target·m^2), Space: O(n·target·m) | Hard |
